import reducer, { initialState } from "./ordersFeed";
import {
  onOpen,
  onMessage,
  onError,
...
describe("testing OrdersFeed slice", () => {
  ...
  describe("WebSocket actions", () => {
    ...
    it("should update orders on subsequent messages", () => {
      ...
      expect(secondMessage.orders).toEqual(mockOrders);
      expect(secondMessage.total).toBe(100);
      expect(secondMessage.totalToday).toBe(10);
    });
